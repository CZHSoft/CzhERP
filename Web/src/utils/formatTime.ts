/**
 * 时间日期转换
 * @param date 当前时间，new Date() 格式或字符串格式
...
export function formatDate(date: Date | string, format: string = 'YYYY-mm-dd'): string {
	...
	let we = dateObj.getDay(); // 星期
	let z = getWeek(dateObj); // 周
	let qut = Math.floor((dateObj.getMonth() + 3) / 3).toString(); // 季度
	const opt: { [key: string]: string } = {
		'Y+': dateObj.getFullYear().toString(), // 年
		'm+': (dateObj.getMonth() + 1).toString(), // 月(月份从0开始，要+1)
