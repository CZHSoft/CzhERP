<script lang="ts" name="stoStockOutItem" setup>
...
const rules = ref<FormRules>({
  stockOutId: [{required: true, message: '请选择出库单ID！', trigger: 'blur',},],
  ...
  quantity: [{required: true, message: '请选择出库数量！', trigger: 'blur',},],
...
const openDialog = async (row: any, title: string) => {
	state.title = title;
	row = row ?? {  };
	state.ruleForm = row.id ? await stoStockOutItemApi.detail(row.id).then(res => res.data.result) : JSON.parse(JSON.stringify(row));
...
const submit = async () => {
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			await stoStockOutItemApi[state.ruleForm.id ? 'update' : 'add'](values);
			closeDialog();
		} else {
			ElMessage({
...
<template>
	<div class="stoStockOutItem-container">
			...
			<el-form :model="state.ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					<el-form-item v-show="false">
						...
						<el-form-item label="出库单ID" prop="stockOutId">
							<el-input v-model="state.ruleForm.stockOutId" placeholder="请输入出库单ID" show-word-limit clearable />
						</el-form-item>
					</el-col>
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20" >
						...
						<el-form-item label="出库数量" prop="quantity">
							<el-input-number v-model="state.ruleForm.quantity" placeholder="请输入出库数量" clearable />
						</el-form-item>
					</el-col>
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20" >
						...
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20" >
						<el-form-item label="出库库位" prop="locationCode">
							<el-input v-model="state.ruleForm.locationCode" placeholder="请输入出库库位" maxlength="50" show-word-limit clearable />
						</el-form-item>
					</el-col>
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20" >
